...
class Krack_Settings
{
    ...
    public function add_plugin_page()
    {
        ...
        add_options_page( 'Krack Options', 'Krack  Plugin', 'manage_options', 'krack-1234597', array( $this, 'create_admin_page' ) );
    }
    ...
    public function create_admin_page()
    {
        ...
        // Set class property
        $this->options = get_option( 'krack_options' );
        ?>


        <div class="wrap">
            <h2>Marina Settings</h2>        
            <form method="post" action="options.php">
            <?php
                // This prints out all hidden setting fields
                settings_fields( 'krack_options_group' );   
                do_settings_sections( 'krack_options' );
                submit_button(); 
            ?>
            </form>
        </div>
    ...
    {        
        register_setting(
            'krack_options_group',    // Option group
            'krack_options',          // Option name
            array( $this, 'sanitize' ) // Sanitize
        );

        // Paths to Directories
        add_settings_section(
            'krack_paths_section',             // ID
            'Configure user',                  // Title
        ...
        add_settings_field(
            'krack_emails',                     // ID
            'Emails of user (seperate by coma)',                       // Title 
            array( $this, 'input_text_callback' ), // Callback
            'krack_options',                      // Page
            'krack_paths_section',                // Section
            array(                                 // args
                'name' => 'krack_emails',
            )
        );      
/*
    ...
    public function input_text_callback( $args ) {
        printf(
            '<input type="text" id="%s" name="krack_options[%s]" value="%s" />',
